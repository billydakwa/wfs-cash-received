...
Class MembersPage
	Public groups
	Public groupFullChecked
	Public members
	Public sortByGroup
	Public sortOrder
	Public userOrderBy
	Public addSaveButtons
	...
	Public Function init_MembersPage_p1(ByRef params)
		DoAssignmentByRef init_MembersPage_p1,method_MembersPage_MembersPage(me,params)
	End Function
	Public Function commonAssign()
		DoAssignmentByRef commonAssign,method_MembersPage_commonAssign(me)
	End Function
	Public Function save()
		DoAssignmentByRef save,method_MembersPage_save(me)
	End Function
	Public Function fillGridData()
		DoAssignmentByRef fillGridData,method_MembersPage_fillGridData(me)
	End Function
	Public Function DPOrderUsers_p1(ByRef rowInfo)
		DoAssignmentByRef DPOrderUsers_p1,method_MembersPage_DPOrderUsers(me,rowInfo)
	End Function
	Public Function fillMembers()
		DoAssignmentByRef fillMembers,method_MembersPage_fillMembers(me)
	End Function
	Public Function fillGroups()
		DoAssignmentByRef fillGroups,method_MembersPage_fillGroups(me)
	End Function
	Public Function sortGroups_p1(ByRef smartyGroups)
		DoAssignmentByRef sortGroups_p1,method_MembersPage_sortGroups(me,smartyGroups)
	End Function
	Public Function prepareForResizeColumns()
		DoAssignmentByRef prepareForResizeColumns,method_MembersPage_prepareForResizeColumns(me)
	End Function
	Public Function rulePRG()
		DoAssignmentByRef rulePRG,method_MembersPage_rulePRG(me)
	End Function
	Public Function setSessionVariables()
		DoAssignmentByRef setSessionVariables,method_MembersPage_setSessionVariables(me)
	End Function
	Public Function doSortByGroup_p1(ByRef rowInfo)
		DoAssignmentByRef doSortByGroup_p1,method_MembersPage_doSortByGroup(me,rowInfo)
	End Function
	Public Function buildOrderParams()
		DoAssignmentByRef buildOrderParams,method_MembersPage_buildOrderParams(me)
	End Function
	Public Function prepareForBuildPage()
		DoAssignmentByRef prepareForBuildPage,method_MembersPage_prepareForBuildPage(me)
	End Function
	Public Function showPage()
		DoAssignmentByRef showPage,method_MembersPage_showPage(me)
	End Function
...
' serialize stuff
	Public Function ASPserialize
		...
		setArrElement out,"groups", groups
		setArrElement out,"groupFullChecked", groupFullChecked
		setArrElement out,"members", members
		setArrElement out,"sortByGroup", sortByGroup
		setArrElement out,"sortOrder", sortOrder
		setArrElement out,"userOrderBy", userOrderBy
		setArrElement out,"addSaveButtons", addSaveButtons
	...
	Public Sub ASPunserialize(dict)
		DoAssignment groups, dict("groups")
		DoAssignment groupFullChecked, dict("groupFullChecked")
		DoAssignment members, dict("members")
		DoAssignment sortByGroup, dict("sortByGroup")
		DoAssignment sortOrder, dict("sortOrder")
		DoAssignment userOrderBy, dict("userOrderBy")
		DoAssignment addSaveButtons, dict("addSaveButtons")
...
' end serialize
End Class
' MembersPage implementation 
Function method_MembersPage_MembersPage(byref this_object,ByRef params)
	doClassAssignment this_object,"groups",CreateDictionary()
	doClassAssignment this_object,"groupFullChecked",CreateDictionary()
	doClassAssignment this_object,"members",CreateDictionary()
	this_object.sortByGroup = false
	this_object.sortOrder = ""
	this_object.userOrderBy = ""
	this_object.addSaveButtons = false
	...
	method_ListPage_Simple_ListPage_Simple this_object,params
	this_object.templatefile = "admin_members_list.htm"
	this_object.listAjax = false
End Function
Function method_MembersPage_commonAssign(byref this_object)
	Dim userheaderlink_attrs
	method_ListPage_Simple_commonAssign this_object
	...
	if bValue(this_object.addSaveButtons) then
		this_object.xt.assign_p2 "recordcontrols_block",true
		this_object.xt.assign_p2 "savebuttons_block",true
		this_object.xt.assign_p2 "savebutton_attrs",("id=""saveButton" & CSmartStr(this_object.id)) & """"
		this_object.xt.assign_p2 "resetbutton_attrs",("id=""resetButton" & CSmartStr(this_object.id)) & """"
	end if
	...
	userheaderlink_attrs = ("href=""" & CSmartStr(this_object.tName)) & "_list.asp?orderby="
	if not IsFalse(this_object.sortByGroup) or not IsEqual(this_object.userOrderBy,"A") then
		userheaderlink_attrs = CSmartStr(userheaderlink_attrs) & "A"
	else
		userheaderlink_attrs = CSmartStr(userheaderlink_attrs) & "D"
	end if
	userheaderlink_attrs = CSmartStr(userheaderlink_attrs) & """"
	this_object.xt.assign_p2 "userheaderlink_attrs",userheaderlink_attrs
	if IsFalse(this_object.sortByGroup) then
		if IsEqual(this_object.userOrderBy,"A") then
			if bValue(this_object.is508) then
				this_object.xt.assign_p2 "users_orderimg","<img src=""images/up.gif"" alt="" "" border=0>"
			else
				this_object.xt.assign_p2 "users_orderimg","<img src=""images/up.gif"" border=0>"
			end if
		else
			if IsEqual(this_object.userOrderBy,"D") then
				if bValue(this_object.is508) then
					this_object.xt.assign_p2 "users_orderimg","<img src=""images/down.gif"" alt="" "" border=0>"
				else
					this_object.xt.assign_p2 "users_orderimg","<img src=""images/down.gif"" border=0>"
				end if
			end if
		end if
	end if
...
End Function
Function method_MembersPage_save(byref this_object)
	Dim useridx,sql,user,var_SESSION,var_POST,glist,g
	if IsEqual(postvalue("a"),"save") then
		useridx = 0
		GetCollectionBounds postvalue("username"),c_memberspage_loopIdx2,c_memberspage_loopMax2
		do while c_memberspage_loopIdx2<=c_memberspage_loopMax2
			c_memberspage_arrKey2 = GetCollectionKey(postvalue("username"),c_memberspage_loopIdx2)
			doAssignment user,ArrayElement(postvalue("username"),c_memberspage_arrKey2)
			useridx = CSmartDbl(useridx)+1
			sql = ("delete from [ugmembers] where UserName='" & CSmartStr(db_addslashes(user))) & "'"
			if IsEqual(user,Session("UserID")) then
				sql = CSmartStr(sql) & " and GroupID<>-1"
			end if
			db_exec sql,this_object.conn
			if bValue(asp_count(GetRequestValue(RequestForm(),"cb_" & CSmartStr(useridx)))) then
				glist = ""
				GetCollectionBounds GetRequestValue(RequestForm(),"cb_" & CSmartStr(useridx)),c_memberspage_loopIdx3,c_memberspage_loopMax3
				c_memberspage_exitLoop3=false
				do while c_memberspage_loopIdx3<=c_memberspage_loopMax3
					c_memberspage_exitLoop3=false
					do
						c_memberspage_arrKey3 = GetCollectionKey(GetRequestValue(RequestForm(),"cb_" & CSmartStr(useridx)),c_memberspage_loopIdx3)
						doAssignment g,GetRequestValue(GetRequestValue(RequestForm(),"cb_" & CSmartStr(useridx)),c_memberspage_arrKey3)
						if IsLess(g,0) then
							if IsEqual(user,Session("UserID")) and IsEqual(g,-1) then
								exit do
							end if
							sql = ((("insert into [ugmembers] (UserName,GroupID) values ('" & CSmartStr(htmlspecialchars(user))) & "',") & CSmartStr(g)) & ")"
							db_exec sql,this_object.conn
						else
							if not IsEqual(glist,"") then
								glist = CSmartStr(glist) & ","
							end if
							glist = CSmartStr(glist) & CSmartStr(g)
						end if
					loop while false
					if c_memberspage_exitLoop3 then _
						exit do
					c_memberspage_loopIdx3=c_memberspage_loopIdx3+1
				loop
				if not IsEqual(glist,"") then
					sql = ((("insert into [ugmembers] (UserName,GroupID) select '" & CSmartStr(htmlspecialchars(user))) & "',GroupID from [uggroups] where GroupID in (") & CSmartStr(glist)) & ")"
					db_exec sql,this_object.conn
				end if
			end if
			c_memberspage_loopIdx2=c_memberspage_loopIdx2+1
		loop
	end if
End Function
Function method_MembersPage_fillGridData(byref this_object)
	Dim rowInfo,shade,data,this,row,member_indexes,m,idx,rowgroups,userfullchecked,checked,smarty_group,i,g,groupfullchecked,usercheckbox_attrs,smartyGroups
	Set rowInfo = (CreateDictionary())
	shade = false
	doAssignmentByRef data,this_object.beforeProccessRow()
	do while bValue(data) and (not IsFalse(this_object.sortByGroup) or IsLessOrEqual(this_object.recNo,this_object.pageSize))
		...
		setArrElement row,"rowattrs",CSmartStr(ArrayElement(row,"rowattrs")) & ((" rowid=""" & CSmartStr(this_object.recNo)) & """")
		setArrElement row,"grid_record",CreateDictionary()
		setArrElementN row,CreateArray2("grid_record","data"),CreateDictionary()
		Set member_indexes = (CreateDictionary())
		GetCollectionBounds this_object.members,c_memberspage_loopIdx5,c_memberspage_loopMax5
		do while c_memberspage_loopIdx5<=c_memberspage_loopMax5
			idx = GetCollectionKey(this_object.members,c_memberspage_loopIdx5)
			doAssignment m,ArrayElement(this_object.members,idx)
			if IsEqual(ArrayElement(m,1),ArrayElement(data,"")) then
				setArrElement member_indexes,asp_count(member_indexes),idx
			end if
			c_memberspage_loopIdx5=c_memberspage_loopIdx5+1
		loop
		Set rowgroups = (CreateDictionary())
		userfullchecked = true
		GetCollectionBounds this_object.groups,c_memberspage_loopIdx6,c_memberspage_loopMax6
		do while c_memberspage_loopIdx6<=c_memberspage_loopMax6
			idx = GetCollectionKey(this_object.groups,c_memberspage_loopIdx6)
			doAssignment g,ArrayElement(this_object.groups,idx)
			checked = false
			Set smarty_group = (CreateDictionary())
			GetCollectionBounds member_indexes,c_memberspage_loopIdx7,c_memberspage_loopMax7
			do while c_memberspage_loopIdx7<=c_memberspage_loopMax7
				c_memberspage_arrKey7 = GetCollectionKey(member_indexes,c_memberspage_loopIdx7)
				doAssignment i,ArrayElement(member_indexes,c_memberspage_arrKey7)
				if IsEqual(ArrayElement(ArrayElement(this_object.members,i),0),ArrayElement(g,0)) then
					checked = true
					exit do
				end if
				c_memberspage_loopIdx7=c_memberspage_loopIdx7+1
			loop
			setArrElement smarty_group,"groupbox_attrs",((("name=""cb_" & CSmartStr(this_object.recNo)) & "[]"" value=""") & CSmartStr(ArrayElement(g,0))) & """"
			setArrElement smarty_group,"checked",0
			if not bValue(checked) then
				userfullchecked = false
				setArrElement groupfullchecked,idx,false
			else
				setArrElement groupfullchecked,idx,true
				setArrElement smarty_group,"groupbox_attrs",CSmartStr(ArrayElement(smarty_group,"groupbox_attrs")) & " checked"
				setArrElement smarty_group,"checked",1
			end if
			setArrElement smarty_group,"group",ArrayElement(g,0)
			setArrElement rowgroups,asp_count(rowgroups),smarty_group
			c_memberspage_loopIdx6=c_memberspage_loopIdx6+1
		loop
		setArrElement row,"usergroup_boxes",CreateDictionary1("data",rowgroups)
		usercheckbox_attrs = ""
		if bValue(userfullchecked) then
			usercheckbox_attrs = CSmartStr(usercheckbox_attrs) & " checked"
		end if
		setArrElement row,"begin",("<input type=hidden name=""username[]"" value=""" & CSmartStr(htmlspecialchars(ArrayElement(data,"")))) & """>"
		setArrElement row,"username",htmlspecialchars(ArrayElement(data,""))
		setArrElement row,"user",ArrayElement(data,"")
		usercheckbox_attrs = CSmartStr(usercheckbox_attrs) & ((" name=""user_" & CSmartStr(htmlspecialchars(ArrayElement(data,"")))) & """")
		usercheckbox_attrs = CSmartStr(usercheckbox_attrs) & " onclick=""var ch=document.getElementsByName('cb_'+$(this).attr('rowid')+'[]');" & vbcrlf & _
			"				for(i=0;i<ch.length;i++)" & vbcrlf & _
			"					ch[i].checked=this.checked;" & vbcrlf & _
			"				return true;"""
		if IsFalse(this_object.sortByGroup) then
			usercheckbox_attrs = CSmartStr(usercheckbox_attrs) & (" rowid=" & CSmartStr(this_object.recNo))
		end if
		setArrElement row,"usercheckbox_attrs",usercheckbox_attrs
		setArrElement row,"recNo",this_object.recNo
		...
		setArrElement rowInfo,asp_count(rowInfo),row
		doAssignmentByRef data,this_object.beforeProccessRow()
	loop
	GetCollectionBounds this_object.groups,c_memberspage_loopIdx9,c_memberspage_loopMax9
	do while c_memberspage_loopIdx9<=c_memberspage_loopMax9
		c_memberspage_arrKey9 = GetCollectionKey(this_object.groups,c_memberspage_loopIdx9)
		doAssignment g,ArrayElement(this_object.groups,c_memberspage_arrKey9)
		setArrElement smartyGroups,asp_count(smartyGroups),CreateDictionary3("groupname",htmlspecialchars(ArrayElement(g,1)),"groupheaderlink_attrs",((("href=""" & CSmartStr(this_object.tName)) & "_list.asp?orderby=a") & CSmartStr(ArrayElement(g,0))) & """","groupheaderbox_attrs",("name=""" & CSmartStr(ArrayElement(g,0))) & """ onclick=""var chk = this.checked; $('input[@type=checkbox][@value\='+this.name+'][@name^=cb_]').each( function(){this.checked=chk;})""")
		c_memberspage_loopIdx9=c_memberspage_loopIdx9+1
	loop
	GetCollectionBounds smartyGroups,c_memberspage_loopIdx10,c_memberspage_loopMax10
	do while c_memberspage_loopIdx10<=c_memberspage_loopMax10
		idx = GetCollectionKey(smartyGroups,c_memberspage_loopIdx10)
		doAssignment g,ArrayElement(smartyGroups,idx)
		if not IsEmpty(ArrayElement(groupfullchecked,idx)) and bValue(ArrayElement(groupfullchecked,idx)) then
			setArrElementN smartyGroups,CreateArray2(idx,"groupheaderbox_attrs"),CSmartStr(ArrayElement(ArrayElement(smartyGroups,idx),"groupheaderbox_attrs")) & " checked"
		end if
		c_memberspage_loopIdx10=c_memberspage_loopIdx10+1
	loop
	this_object.sortGroups_p1 smartyGroups
	this_object.doSortByGroup_p1 rowInfo
	this_object.xt.assign_loopsection_p2 "grid_row",rowInfo
	this_object.xt.assign_loopsection_p2 "usergroup_header",smartyGroups
	if bValue(asp_count(rowInfo)) then
		this_object.addSaveButtons = true
	end if
End Function
Function method_MembersPage_DPOrderUsers(byref this_object,ByRef rowInfo)
	doAssignment sortgroup,this_object.sortByGroup
	doAssignment sortorder,this_object.sortOrder
	sortMembers rowInfo
End Function
Function method_MembersPage_fillMembers(byref this_object)
	Dim trs,tdata
	doAssignmentByRef trs,db_query("select UserName,GroupID from [ugmembers] order by UserName,GroupID",this_object.conn)
	do while bValue(doAssignmentByRef(tdata,db_fetch_numarray(trs)))
		setArrElement this_object.members,asp_count(this_object.members),CreateDictionary2(Empty,ArrayElement(tdata,1),Empty,ArrayElement(tdata,0))
	loop
End Function
Function method_MembersPage_fillGroups(byref this_object)
	Dim trs,tdata
	setArrElement this_object.groups,asp_count(this_object.groups),CreateDictionary2(Empty,-1,Empty,("<" & CSmartStr("Admin")) & ">")
	setArrElement this_object.groupFullChecked,asp_count(this_object.groupFullChecked),true
	doAssignmentByRef trs,db_query("select GroupID,Label from [uggroups] order by Label",this_object.conn)
	do while bValue(doAssignmentByRef(tdata,db_fetch_numarray(trs)))
		setArrElement this_object.groups,asp_count(this_object.groups),CreateDictionary2(Empty,ArrayElement(tdata,0),Empty,ArrayElement(tdata,1))
		setArrElement this_object.groupFullChecked,asp_count(this_object.groupFullChecked),true
	loop
End Function
Function method_MembersPage_sortGroups(byref this_object,ByRef smartyGroups)
	Dim g,i
	GetCollectionBounds this_object.groups,c_memberspage_loopIdx13,c_memberspage_loopMax13
	do while c_memberspage_loopIdx13<=c_memberspage_loopMax13
		i = GetCollectionKey(this_object.groups,c_memberspage_loopIdx13)
		doAssignment g,ArrayElement(this_object.groups,i)
		if IsEqual(this_object.sortByGroup,ArrayElement(g,0)) and IsEqual(this_object.sortOrder,"a") then
			setArrElementN smartyGroups,CreateArray2(i,"groupheaderlink_attrs"),((("href=""" & CSmartStr(this_object.tName)) & "_list.asp?orderby=d") & CSmartStr(ArrayElement(g,0))) & """"
			if bValue(this_object.is508) then
				setArrElementN smartyGroups,CreateArray2(i,"groupheader_img"),"<img src=""images/up.gif"" alt="" "" border=0>"
			else
				setArrElementN smartyGroups,CreateArray2(i,"groupheader_img"),"<img src=""images/up.gif"" border=0>"
			end if
		else
			if IsEqual(this_object.sortByGroup,ArrayElement(g,0)) and IsEqual(this_object.sortOrder,"d") then
				if bValue(this_object.is508) then
					setArrElementN smartyGroups,CreateArray2(i,"groupheader_img"),"<img src=""images/down.gif"" alt="" "" border=0>"
				else
					setArrElementN smartyGroups,CreateArray2(i,"groupheader_img"),"<img src=""images/down.gif"" border=0>"
				end if
			end if
		end if
		c_memberspage_loopIdx13=c_memberspage_loopIdx13+1
	loop
End Function
Function method_MembersPage_prepareForResizeColumns(byref this_object)
	method_MembersPage_prepareForResizeColumns = true
	Exit Function
End Function
Function method_MembersPage_rulePRG(byref this_object)
	Dim this
...
End Function
Function method_MembersPage_setSessionVariables(byref this_object)
	Dim var_SESSION
	method_ListPage_setSessionVariables this_object
	if (not bValue(Session(CSmartStr(this_object.sessionPrefix) & "_orderby")) or IsEqual(this_object.userOrderBy,"A")) or IsEqual(this_object.userOrderBy,"D") then
		this_object.sortByGroup = false
	end if
	doClassAssignment this_object,"sortByGroup",asp_substr(Session(CSmartStr(this_object.sessionPrefix) & "_orderby"),1,empty)
	doClassAssignment this_object,"sortOrder",asp_substr(Session(CSmartStr(this_object.sessionPrefix) & "_orderby"),0,1)
	if IsEqual(Session(CSmartStr(this_object.sessionPrefix) & "_orderby"),"D") then
		this_object.userOrderBy = "D"
	else
		this_object.userOrderBy = "A"
	end if
End Function
Function method_MembersPage_doSortByGroup(byref this_object,ByRef rowInfo)
	Dim this,firstindex,i,j
	if not IsFalse(this_object.sortByGroup) then
		this_object.DPOrderUsers_p1 rowInfo
		firstindex = CSmartDbl(this_object.pageSize)*(CSmartDbl(this_object.myPage)-1)
		i = 0
		do while IsLess(i,firstindex)
			asp_array_shift rowInfo
			i = CSmartDbl(i)+1
		loop
		if IsLess(this_object.pageSize,asp_count(rowInfo)) then
			asp_array_splice rowInfo,this_object.pageSize
		end if
		this_object.recNo = 1
		i = 0
		do while IsLess(i,asp_count(rowInfo))
			setArrElementN rowInfo,CreateArray2(i,"usercheckbox_attrs"),CSmartStr(ArrayElement(ArrayElement(rowInfo,i),"usercheckbox_attrs")) & (" rowid=" & CSmartStr(this_object.recNo))
			j = 0
			do while IsLess(j,asp_count(ArrayElement(ArrayElement(ArrayElement(rowInfo,i),"usergroup_boxes"),"data")))
				setArrElementN rowInfo,CreateArray5(i,"usergroup_boxes","data",j,"groupbox_attrs"),((("name=""cb_" & CSmartStr(this_object.recNo)) & "[]"" value=""") & CSmartStr(ArrayElement(ArrayElement(ArrayElement(ArrayElement(ArrayElement(rowInfo,i),"usergroup_boxes"),"data"),j),"group"))) & """"
				if bValue(ArrayElement(ArrayElement(ArrayElement(ArrayElement(ArrayElement(rowInfo,i),"usergroup_boxes"),"data"),j),"checked")) then
					setArrElementN rowInfo,CreateArray5(i,"usergroup_boxes","data",j,"groupbox_attrs"),CSmartStr(ArrayElement(ArrayElement(ArrayElement(ArrayElement(ArrayElement(rowInfo,i),"usergroup_boxes"),"data"),j),"groupbox_attrs")) & " checked"
				end if
				j = CSmartDbl(j)+1
			loop
			this_object.recNo = CSmartDbl(this_object.recNo)+1
...
End Function
Function method_MembersPage_buildOrderParams(byref this_object)
	if IsEqual(this_object.userOrderBy,"D") then
		this_object.strOrderBy = "Order by 2 desc"
	else
		this_object.strOrderBy = "Order by 2 asc"
	end if
End Function
Function method_MembersPage_prepareForBuildPage(byref this_object)
	Dim this
	this_object.save 
	this_object.rulePRG 
	this_object.buildOrderParams 
	this_object.fillMembers 
	this_object.fillGroups 
	this_object.buildSQL 
	this_object.buildPagination 
	if IsFalse(this_object.sortByGroup) then
		this_object.seekPageInRecSet_p1 this_object.querySQL
	else
		doClassAssignment this_object,"recSet",db_query(this_object.querySQL,this_object.conn)
	end if
...
End Function
Function method_MembersPage_showPage(byref this_object)
